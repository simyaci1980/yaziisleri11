...
def sorular(request):
    ...
    json_path = os.path.join(settings.BASE_DIR, 'http://127.0.0.1:8000', 'static', 'data', 'sorular.json')

    try:
        with open(json_path, 'r', encoding='utf-8') as file:
            data = json.load(file)  # JSON dosyasını oku
        return JsonResponse(data, safe=False)  # JSON formatında döndür
